...
class CommonController extends Controller
{
	...
	{   
	   ...
	   $begin_key = ltrim($value, $first_key);
	   $record = array('value' => $value, 'first_key' => $first_key, 'begin_key' => $begin_key);
	   return view('test')->with($record);
	}
				...
				if($result_count != 0)
				{
				   ...
				   $item_price = $this->price_info($item['view']->item_flash,$item['view']->regular_price);
					  ...
					  if($additional['setting']->subscription_mode == 0)
					  {
					   ...
					   $savedata = array('session_id' => $session_id, 'item_id' => $item_id, 'item_name' => $item_name, 'item_user_id' => $item_user_id, 'item_token' => $item_token, 'license' => $license, 'start_date' => $start_date, 'end_date' => $end_date, 'item_price' => $price, 'vendor_amount' => $vendor_amount, 'admin_amount' => $admin_amount, 'total_price' => $price, 'order_status' => $order_status, 'item_serial_stock' => 1);
					   
					   
					   $updatedata = array('license' => $license, 'start_date' => $start_date, 'end_date' => $end_date, 'item_price' => $price, 'total_price' => $price, 'item_serial_stock' => 1);
	   ...
	   foreach( $codes as $index => $code ) 
	   {
	      if($code > 0)
		  {
				...
				else
				{
	   			...
	   			$updatedata = array('coupon_key' => $coupon_key, 'coupon_id' => $coupon_id, 'coupon_code' => $coupon_code, 'coupon_type' => $coupon_type, 'coupon_value' => $coupon_value, 'discount_price' => $coupon_price,'item_serial_stock' => $quantity, 'vendor_amount' => $vendor_amount, 'admin_amount' => $admin_amount, 'total_price' => $price);        
	...
	public function view_cart(Request $request)
	{
	  ...
	  $split = explode("_", $item_price);
       ...
       $price = base64_decode($split[0]) * $qty;
	   $license = $split[1];
	   if($license == 'regular')
	   {
	     $start_date = date('Y-m-d');
	   ...
	   $savedata = array('session_id' => $session_id, 'item_id' => $item_id, 'item_name' => $item_name, 'item_user_id' => $item_user_id, 'item_token' => $item_token, 'license' => $license, 'start_date' => $start_date, 'end_date' => $end_date, 'item_price' => $itemprice, 'vendor_amount' => $vendor_amount, 'admin_amount' => $admin_amount, 'total_price' => $price, 'order_status' => $order_status, 'coupon_key' => $coupon_key, 'coupon_id' => $coupon_id, 'coupon_code' => $coupon_code, 'coupon_type' => $coupon_type, 'coupon_value' => $coupon_value, 'discount_price' => $coupon_price, 'item_serial_stock' => $qty);
	   
	   
	   $updatedata = array('license' => $license, 'start_date' => $start_date, 'end_date' => $end_date, 'item_price' => $itemprice, 'total_price' => $price, 'vendor_amount' => $vendor_amount, 'admin_amount' => $admin_amount, 'coupon_key' => $coupon_key, 'coupon_id' => $coupon_id, 'coupon_code' => $coupon_code, 'coupon_type' => $coupon_type, 'coupon_value' => $coupon_value, 'discount_price' => $coupon_price, 'item_serial_stock' => $qty);
	  ...
	  if($search_keyword != "" ||  $orderby != "" || $min_price != "" || $max_price != "")
	  {
		  ...
		  if($additional['settings']->subscription_mode == 1)
		  {
			  $itemData['item'] = Items::with('ratings')
			                  ->select('items.item_id','items.item_liked','items.item_slug','items.item_preview','items.item_name','items.item_type','users.user_photo','users.username','users.user_document_verified','items.updated_item','items.item_sold','items.free_download','items.item_flash','items.regular_price','items.item_token','items.user_id','items.file_type','items.item_delimiter','items.item_serials_list') 
