              ...
              <div class="row mx-n2 pt-2">
                <div class="col-md-4 col-sm-12 px-2 mb-4">
                  <div class="bg-secondary h-100 rounded-lg p-4 text-center">
                    ...
                    <p class="h2 mb-2">{{ Helper::price_format($allsettings->site_currency_position,$total_sale,"symbol") }}</p>
                  </div>
                </div>
                <div class="col-md-4 col-sm-12 px-2 mb-4">
                  <div class="bg-secondary h-100 rounded-lg p-4 text-center">
                    <h3 class="font-size-sm text-muted">{{ __('Total Purchases') }}</h3>
                    <p class="h2 mb-2">{{ Helper::price_format($allsettings->site_currency_position,$purchase_sale,"symbol") }}</p>
                  </div>
                </div>
                <div class="col-md-4 col-sm-12 px-2 mb-4">
                  <div class="bg-secondary h-100 rounded-lg p-4 text-center">
                    <h3 class="font-size-sm text-muted">{{ __('Total Credited') }}</h3>
                    <p class="h2 mb-2">{{ Helper::price_format($allsettings->site_currency_position,$credit_amount,"symbol") }}</p>
                  </div>
                </div>
                <div class="col-md-4 col-sm-12 px-2 mb-4">
                  <div class="bg-secondary h-100 rounded-lg p-4 text-center">
                    <h3 class="font-size-sm text-muted">{{ __('Total Withdraw') }}</h3>
                    <p class="h2 mb-2">{{ Helper::price_format($allsettings->site_currency_position,$drawal_amount,"symbol") }}</p>
                  </div>
                </div>
                <div class="col-md-4 col-sm-12 px-2 mb-4">
                  <div class="bg-secondary h-100 rounded-lg p-4 text-center">
                    <h3 class="font-size-sm text-muted">{{ __('Referral Commission') }}</h3>
                    <p class="h2 mb-2">{{ Helper::price_format($allsettings->site_currency_position,Auth::user()->referral_amount,"symbol") }}</p>
                  </div>
                </div>
                <div class="col-md-4 col-sm-12 px-2 mb-4">
                  <div class="bg-secondary h-100 rounded-lg p-4 text-center">
                            ...
                            <table class="table">
                                ...
                                @foreach($orderData['item'] as $item)
                                    <tr class="prod-item">
                                        ...
                                        <td>{{ Helper::price_format($allsettings->site_currency_position,$item->total,"symbol") }}</td>
                                        <td class="earning theme-color">{{ Helper::price_format($allsettings->site_currency_position,$item->vendor_amount,"symbol") }}</td>
                                        <td>
