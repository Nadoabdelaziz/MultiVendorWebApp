            ...
            <tr>
              ...
              <td style="width:20%;">
                <div class="currency">
                  ...
                  <span class="pricemove">{{ $allsettings->site_currency_symbol }}</span><input type="text" name="price[]" id="coster" class="price" value="{{ $cart->item_price }}" readonly> 
                  @else
                  <span class="pricemove"><input type="text" name="price[]" id="coster-right" class="price" value="{{ $cart->item_price }}" readonly>{{ $allsettings->site_currency_symbol }}</span> 
                  @endif
                </div>
              </td>
             ...
             @endif
              <td style="width:20%;">
                <div class="currency">
                 @if($allsettings->site_currency_position == "left")
                  <span class="pricemove">{{ $allsettings->site_currency_symbol }}</span><input type="text" name="cost" class="cost" id="coster" value="" readonly>
                  @else
                  <span class="pricemove"><input type="text" name="cost" class="cost" id="coster-right" value="" readonly>{{ $allsettings->site_currency_symbol }}</span>
                  @endif
                </div>
              </td>
              <td style="width:10%;">
          ...
          <aside class="col-lg-4">
            ...
            <div class="cz-sidebar-static h-100 ml-auto border-left">
              <ul class="list-unstyled font-size-sm pt-3 pb-2 border-bottom">
                  ...
                  @endphp
                  @if($extra_fee != 0)
                  <li class="d-flex justify-content-between align-items-center"><span class="mr-2">{{ __('Processing Fee') }}</span><span class="text-right"><input type="text" name="extra_fee" class="extra_fee" id="cart_subtotal" value="{{ $extra_fee }}" readonly></span></li>
                  @endif
                  @if($coupon_code != "")
                  @php 
...
$(function() {
  ...
  function calculate() {
	  ...
	  if(types == "left")
	  {
      $('.subtotal').val("{{ $allsettings->site_currency_symbol }}"+parseFloat(subtotalVal).toFixed(2));
	  }
	  else
	  {
	  $('.subtotal').val(parseFloat(subtotalVal).toFixed(2)+"{{ $allsettings->site_currency_symbol }}");
	  }
      var extra_fee_type = "{{ $addition_settings->site_extra_fee_type }}";
	  var extra_fee_price = "{{ $allsettings->site_extra_fee }}";
      if(extra_fee_type == 'fixed')
      {
         if(types == "left")
		 {            
		 $(".extra_fee").val("{{ $allsettings->site_currency_symbol }}"+parseFloat(extra_fee_price).toFixed(2));
		 }
		 else
		 {
		 $(".extra_fee").val(parseFloat(extra_fee_price).toFixed(2)+"{{ $allsettings->site_currency_symbol }}");
		 }
      ...
      else
      {
	      if(types == "left")
		  {
          $(".extra_fee").val("{{ $allsettings->site_currency_symbol }}"+((subtotalVal / 100) * extra_fee_price).toFixed(2));
		  }
		  else
		  {
		  $(".extra_fee").val(((subtotalVal / 100) * extra_fee_price).toFixed(2)+"{{ $allsettings->site_currency_symbol }}");
		  }
	  ...
	  if(coupon_type == 'fixed')
	  {
	    if(types == "left")
		{
	    $(".coupon_discount").val(" - {{ $allsettings->site_currency_symbol }}"+parseFloat(coupon_value).toFixed(2));
		}
		else
		{
		$(".coupon_discount").val(" - "+parseFloat(coupon_value).toFixed(2)+"{{ $allsettings->site_currency_symbol }}");
		}
	  ...
	  else if(coupon_type == 'percentage')
      {
	     if(types == "left")
		 {
	     $(".coupon_discount").val(" - {{ $allsettings->site_currency_symbol }}"+((subtotalVal / 100) * coupon_value).toFixed(2));
		 }
		 else
		 {
		 $(".coupon_discount").val(" - "+((subtotalVal / 100) * coupon_value).toFixed(2)+"{{ $allsettings->site_currency_symbol }}");
		 }
	  ...
	  if(types == "left")
	  {
      $(".total").val("{{ $allsettings->site_currency_symbol }}"+parseFloat(total).toFixed(2));
	  }
	  else
	  {
	  $(".total").val(parseFloat(total).toFixed(2)+"{{ $allsettings->site_currency_symbol }}");
	  }
  }
