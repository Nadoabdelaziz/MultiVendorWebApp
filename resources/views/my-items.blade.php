        ...
        <div class="row">
          ...
          <section class="col-lg-8 pt-lg-4 pb-4 mb-3">
            <div class="pt-2 px-4 pl-lg-0 pr-xl-5">
              ...
              <h2 class="h3 pt-2 pb-4 mb-0 col pull-left">{{ __('Manage Items') }}<?php /*?><span class="badge badge-secondary font-size-sm text-body align-middle ml-2">{{ count($itemData['item']) }}</span><?php */?></h2>         <div class="col pull-right">
              ...
              <span class="close-floating" data-toggle="tooltip" title="{{ __('Remove from favourites') }}"><i class="dwg-close"></i></span>
                <div class="media-body text-center text-sm-left">
                  ...
                  <div class="d-inline-block text-accent">@if($featured->free_download == 0){{ Helper::price_format($allsettings->site_currency_position,$price,"symbol") }}@else<span>{{ __('Free') }}</span>@endif</div><a class="d-inline-block text-accent font-size-ms border-left ml-2 pl-2" href="{{ URL::to('/shop') }}/item-type/{{ $featured->item_type }}">{{ str_replace('-',' ',$featured->item_type) }}</a>
